...
func main() {
	...
	consensusNetwork := consensus.NewConsensusNetwork()
	...
	// Initialize KeyManager (pass nil or a real VaultClient as needed)
	keyManager := keyvault.NewKeyManager(nil)
	...
	// Initialize logger
	logger, err := api.NewLogger(config.LogPath, "api")
	if err != nil {
		log.Fatalf("Failed to initialize logger: %v", err)
	}

	// Initialize rate limiter
	rateLimiter := api.NewRateLimiter(config.RateLimitRequests, time.Duration(config.RateLimitWindowSecs)*time.Second)
	...
	authService := api.NewPQAuthService(authConfig, keyManager)

	// Create API handler
	handler := api.NewHandler(consensusNetwork, identityService, contentProcessor, authService)

	// Create and start server with all required dependencies
	server, err := api.NewServer(handler, keyManager, config, logger, rateLimiter)
	if err != nil {
