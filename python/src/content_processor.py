...
# Try to import enhanced entity extraction
try:
    from entity_extraction_enhanced import EnhancedEntityExtractor
    ENHANCED_ENTITY_EXTRACTION_AVAILABLE = True
except ImportError:
    ENHANCED_ENTITY_EXTRACTION_AVAILABLE = False

# Try to import improved claim detection
try:
...
# Try to import external trusted source verification
try:
...
class ContentProcessor:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        # Initialize enhanced entity extractor if available
        self.entity_extractor = None
        if ENHANCED_ENTITY_EXTRACTION_AVAILABLE:
            try:
                self.entity_extractor = EnhancedEntityExtractor(config)
                print("Enhanced entity extraction initialized")
            except Exception as e:
                print(f"Error initializing enhanced entity extraction: {e}")

        # Initialize improved claim detector if available
        self.claim_detector = None
        if IMPROVED_CLAIM_DETECTION_AVAILABLE:
            try:
                self.claim_detector = ClaimDetector(config)
                print("Improved claim detection initialized")
            except Exception as e:
                print(f"Error initializing improved claim detection: {e}")
    ...
    def _extract_entities(self, news_content: NewsContent) -> None:
        ...
        if self.entity_extractor is not None:
            try:
                ...
                entities = self.entity_extractor.extract_entities(news_content)
                news_content.entities = entities
                return
            except Exception as e:
                print(f"Error using enhanced entity extraction: {e}")
                # Fall back to basic extraction

        # Basic entity extraction using NLTK
        entities = []
    ...
    def _extract_claims(self, news_content: NewsContent) -> None:
        ...
        # Use improved claim detection if available
        if self.claim_detector is not None:
            try:
                # Extract claims using the improved detector
                claims = self.claim_detector.extract_claims(news_content)
                news_content.claims = claims
                return
            except Exception as e:
                print(f"Error using improved claim detection: {e}")
                # Fall back to basic extraction

        # Basic claim extraction using simple heuristics
        claims = []
        ...
        for sentence in sentences:
            ...
            is_claim = False
            claim_type = "FACTUAL"  # Updated to match the new claim type system
